...
class PageProcessor:
    ...
    async def process_workflow_page(self, page, file_path: str) -> dict:
        ...
        try:
            ...
            pix.save(image_path)
            doc.close()
            logger.info("Extracted image for page %s", page.page_number)

        except Exception as e:
            logger.error("Failed to extract image from page %s: %s", page.page_number, e)
            raise PageImageExtractionException(
    ...
    async def process_pptx_slide(self, image_path: str, slide_number: int) -> dict:
        ...
        if not os.path.exists(image_path):
            logger.error("Slide image not found for slide %s: %s", slide_number, image_path)
            raise PageImageExtractionException(
    ...
    async def _run_llama_pipeline(self, image_path: str, cleanup: bool) -> dict:
        ...
        try:
            ...
            if cleanup and os.path.exists(image_path):
                ...
                logger.info("Removed temporary image %s", image_path)
        ...
        if workflow.strip():
            ...
            logger.info("Found workflow content in image %s, converting with Azure Workflow", image_path)

            try:
                result = self._converter.convert(workflow)
                parts.append(json.dumps(result.raw_json, ensure_ascii=False, indent=2))
                logger.info("Workflow conversion succeeded for %s", image_path)
            except Exception as e:
                logger.error("Workflow conversion failed for %s: %s", image_path, e)
        ...
        logger.info(
            "Finished processing image %s, content_type=%s, tables=%d",
            image_path,
            content_type,
            len(tables_metadata),
        )
